...
const POTable = () => {
  ...
  const {
    ...
    queryKey: ["activePO"],
    queryFn: () => getActivePOs(`/po/active?perPage=12`, setError),
            ...
            {!activePOList?.length ? (
              ...
              activePOList.map((po) => (
                ...
                >
                  ...
                  <td className="px-6 py-3">{po.deliveryDate.split(" ")[0]}</td>
                  <td className="px-6 py-3">{po.status}</td>
                  <td className="px-6 py-3">{po.processStatus}</td>
                  <td className="px-6 py-3">{po.documentNumber}</td>
                  <td className="px-6 py-3">{po.documentDate.split(" ")[0]}</td>
                  <td className="px-6 py-3">{po.documentTotal}</td>
                </tr>
              ))
            )}
          </tbody>
