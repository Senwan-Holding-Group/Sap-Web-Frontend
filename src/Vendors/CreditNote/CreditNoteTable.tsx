import {  getCreditNoteList } from "@/api/client";
import DataRenderer from "@/components/DataRenderer";
import StatusBadge from "@/components/StatusBadge";
import Pagination from "@/components/ui/Pagination";
import Search from "@/components/ui/Search";
import { useStateContext } from "@/context/useStateContext";
import { creditNoteMenu } from "@/lib/constants";
...
const CreditNoteTable = () => {
   const navigate = useNavigate();
    ...
    const [search, setSearch] = useState({
      searchKey: creditNoteMenu[0].value,
      searchValue: "",
    });
    const {
      data: creditNoteList,
      ...
      queryKey: ["creditNoteList", search, currentPage],
      queryFn: () =>
        getCreditNoteList(
          `/credit-note?${search.searchKey}=${search.searchValue}&perPage=15&page=${currentPage}`,
          setError,
          setTotalPage
        ),
  ...
  return (
    ...
    <div className="flex sm:justify-between items-center flex-col sm:flex-row  gap-4">
      <Search menuList={creditNoteMenu} setSearch={setSearch} search={search} />
    </div>
        ...
        <table className="w-full caption-bottom ">
          ...
          <tbody className="bg-white [&_tr:last-child]:border-0 ">
           {!creditNoteList?.length ? (
              ...
              creditNoteList.map((vendor) => (
            <tr
              key={vendor.documentEntry}
              onClick={() => navigate(`/sap/vendors/creditNoteList/details/${vendor.documentEntry}`)}
            ...
            >
              ...
              <td className="px-6 py-3">{vendor.documentDate.split(" ")[0]} </td>
                
              <td className="px-6 py-3">{numberWithCommas(vendor.documentTotal)} </td>
            </tr>
              )))}
          </tbody>
