import { postAllCreditNote } from "@/api/client";
...
const CalculateCreditNoteAll = ({ disabled, paymentDate }: Props) => {
      ...
      <DialogContent
        ...
        <div className="  flex flex-col items-center justify-center gap-y-4 px-4 h-36 w-full  ">
          <span className="text-base font-normal text-geantSap-gray-500">
            Select a date to calculate credit note
          </span>
          <Popover>
        ...
        <DialogFooter className=" bg-[#fcfcfc] rounded-bl-lg p-6  rounded-br-lg  border-t flex flex-row gap-4 h-[5.5rem] border-geantSap-gray-50">
          ...
          <Button
            ...
            onClick={() => {
              postAllCreditNote(
                `/outgoing-payment/creditNote?selectedDate=${format(
                  selectedDate!,
                  "yyyy-MM-dd"
...
export default CalculateCreditNoteAll;
